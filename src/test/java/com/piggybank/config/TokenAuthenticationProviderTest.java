...
public class TokenAuthenticationProviderTest {
    @InjectMocks
    private TokenAuthenticationProvider sut;
    ...
    public void shouldReturnTheAuthenticatedUserForValidToken() {
        Mockito.when(userAuthenticationService.authenticateByToken(Mockito.anyString()))
                .thenReturn(Optional.of(User.newBuilder()
                        .setUsername("username")
                        .setPassword("password")
                        .build()));


        final UserDetails user = sut.retrieveUser("username", new UsernamePasswordAuthenticationToken("token", "token"));
